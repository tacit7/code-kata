...
> **Serialization:** `run_ruby_tests` must return a `#[derive(Serialize)]` struct so Tauri serializes it to an object over the IPC bridge. Do **not** return a `String` produced by hand-rolled `format!("{{\"status\":...}}")` — that produces a JSON string, not an object; `invoke<{ status: string }>` would then receive a string and type-mismatch at runtime. All paths (ok, timeout, and error) have this bug — `run_ruby_tests` still returns `String`, so the double-encoding applies everywhere, not just error handling. The TypeScript generic above types the already-decoded struct fields directly.
...
- **Native-process prototype (task #8505):** `ca0b31c24` on `worktree-ruby-native-runner-prototype` (PR #710). Blockers 1–3 fixed (`recv_timeout` replaces the sleep+kill watchdog; `env_clear` + minimal env; `OnceLock` Ruby ≥3.0 version gate resolving shims via `RbConfig.ruby`). §9.1a satisfied — `timeout_secs`/`output_cap_bytes` are now invoke params sourced from `ruby-exec-core`. **Still NOT merge-ready — five open issues:**
  1. **Unbounded hang in the ok branch** — `out_rx.recv()` (`lib.rs:253`) blocks until the stdout pipe hits EOF, which any surviving descendant holds open. Verified: a kata calling `spawn("sleep 8")` exits instantly but the pipe stays open 8 s, past the already-satisfied 5 s gate. Fix: use `recv_timeout` on `out_rx`/`err_rx`.
  2. **`kill -9` does not reach descendants** — `Command` creates no process group, so a spawned child survives its parent. Verified: SIGKILLing a ruby that had `spawn("sleep 7")` left the sleep alive, reparented to launchd. Violates R3 and R4. Fix: use `.process_group(0)` and kill the negative pgid.
  3. **§7 serde violation still open** — `run_ruby_tests` returns `String` and hand-rolls `format!` JSON on the ok path (`lib.rs:269`); TS still does `invoke<string>` + `JSON.parse`. Gate §9.1 fails.
  4. **`timeoutSecs` truncation** — TS sends `Math.floor(EXEC_TIMEOUT_MS / 1000)`; any sub-second or non-multiple value silently degrades, and `<1000 ms` yields `recv_timeout(0)`. Pass milliseconds instead.
  5. **Sandboxing unsolved** (unchanged from original blocker 4) — issue 2 above is a concrete demonstration of the cost: the native path cannot currently contain a kata that spawns subprocesses.
