...
> **Serialization:** Rust must return a `serde`-serializable struct and let Tauri handle the IPC encoding. Do **not** hand-roll a JSON string with `format!("{{\"status\":...}}")` — that approach produces double-encoded output (`invoke` would deliver a JSON string rather than an object) and is error-prone in error paths. The TypeScript generic above types the already-decoded object.
...
- **Native-process prototype (task #8505):** Landed — `aa4420b5b` on `worktree-ruby-native-runner-prototype` (PR #710). **Not merge-ready — three open blockers found:**
  ...
  4. **Unsandboxed system Ruby** — spawning the host's system Ruby via `Command::new("ruby")` contradicts `CLAUDE.md:104-106` ("no child processes, no system runtimes") and the packaging assessment's own sidecar-rejection rationale. A vendored or embedded Ruby binary is required.
