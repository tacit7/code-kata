...
- **Native-process prototype (task #8505):** `8b8e36f23` on `worktree-ruby-native-runner-prototype` (PR #710). Clippy clean. Issues 1, 2a, 2b, and 3 all fixed: Ok branch now kills the process group before draining so `spawn("sleep 8")` returns in ~55 ms; `.process_group(0)` + negative-PID kill reaches descendants; `wait-timeout` crate keeps `Child` on the main thread so `Ok(None)` proves the child is alive before `Child::kill()` (race eliminated, not narrowed); `timeout_ms` passed as milliseconds. Ruby discovery now globs versioned dirs. **Still NOT merge-ready — five open issues:**
  1. **Zombie in the `wait_timeout` Err branch** — when `wait_timeout` itself returns `Err(e)` (`lib.rs:298-302`), the code calls `kill_process_group(child_pgid)` and returns without calling `child.wait()`, leaving the child unreaped for the life of the app. The timeout path (`Ok(None)`, `lib.rs:335`) already reaps correctly. This is rare but the only branch that leaks.
  2. **Ok-branch worst-case wall time exceeds §4** — process group killed, then two sequential pipe drains each with a 2 s `recv_timeout`. Worst case: 5 s Ruby + 2 s stdout drain + 2 s stderr drain = 9 s total, against the 5 s `EXEC_TIMEOUT_MS` that §4 now defines as the ceiling for the **entire command**. Fix: either pipeline the drains or make the drain budget part of the 5 s window (e.g. kill at 4 s, 1 s remaining for drain).
  3. **Hardcoded `pipe_drain` constant** — a new duration literal on the Rust side, not sourced from `ruby-exec-core` or passed as an invoke param. Violates §9.1a as now written (any execution-timing constant).
  4. **`kill_process_group` lacks pgid ≤ 1 guard** — before calling `kill(-pgid, SIGKILL)`, the function must verify `pgid > 1` to avoid broadcasting SIGKILL to every process in the session.
  5. **Sandboxing unsolved** (unchanged) — the native path cannot contain a kata that spawns subprocesses.
