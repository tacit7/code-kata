...
> **Serialization:** `run_ruby_tests` must return a `#[derive(Serialize)]` struct so Tauri serializes it to an object over the IPC bridge. Do **not** return a `String` produced by hand-rolled `format!("{{\"status\":...}}")` — that produces a JSON string, not an object; `invoke<{ status: string }>` would then receive a string and type-mismatch at runtime. The current prototype's error paths have this bug. The TypeScript generic above types the already-decoded struct fields directly.
...
- **Native-process prototype (task #8505):** Landed — `aa4420b5b` on `worktree-ruby-native-runner-prototype` (PR #710). **Not merge-ready — four open blockers found:**
  1. **Unconditional `kill -9` with no cancellation path** — the watchdog thread sleeps 5 s and then issues `kill -9` on every run regardless of whether the child already exited. A kata finishing in 150 ms still gets a signal fired at its already-reaped PID 4.85 s later, and a thread is leaked per run. The reaped PID may also be reused by an unrelated process by the time the signal fires. Fix: wait on a channel with `recv_timeout`; issue `kill` only on the timeout branch, and cancel (drop the sender) on normal completion.
  2. **Full parent-env inheritance** — `GEM_HOME`, `RUBYOPT`, and other env vars leak into the child process, allowing the host Ruby environment to alter kata behavior. Reproduced as a hard `load_error` when the host's `GEM_HOME` is set.
  ...
  4. **Unsandboxed system Ruby** — spawning the host's system Ruby via `Command::new("ruby")` contradicts `CLAUDE.md:104-106` ("no child processes, no system runtimes") and the packaging assessment's own sidecar-rejection rationale. Sandboxing is the unsolved open design question for the native path: a vendored binary fixes version pinning and offline availability but does not sandbox anything — kata code would still have full filesystem, network, and exec access. Possible approaches (WASI, macOS `sandbox-exec`, or dropping the sidecar model entirely) need evaluation before this path can be considered.
  Contract issues requiring fixes before merge: `RUBY_TIMEOUT_SECS` and `OUTPUT_CAP_BYTES` hardcoded in `lib.rs` (violates §9.1a). (`{script}` invoke shape already implemented — `native-ruby-runner.ts:63` passes `{ script }`; no violation.)
