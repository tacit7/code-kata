...
- **Native-process prototype (task #8505):** `84396d396` on `worktree-ruby-native-runner-prototype` (PR #710). Clippy clean. Single `run_deadline` bounds total wall time at `timeout_ms`; no Rust-side timing constants; process-group containment via `libc::killpg` with pgid > 1 guard; PID race eliminated via `wait-timeout`; `env_clear`; Ruby >= 3.0 version gate; serde-tagged `RubyRunOutcome`. §4, §7, §9.1, §9.1a all satisfied. Predicted fix landed as described (`2d0cf0598`): one `run_deadline` taken at function entry, budget spent by `wait_timeout` then both drains; `PIPE_DRAIN_TIMEOUT` gone (grepped clean). `6457edaea` additionally: exhausted-drain path correctly returns Timeout rather than load_error; Ruby `OnceLock` warmed in the setup closure so the first kata gets its full budget. `84396d396`: warmup moved to `std::thread::spawn` so window creation is not blocked. **ONE open issue:** sandboxing unsolved — the runner executes untrusted kata code with the user's full filesystem, network and exec authority. Two minor items (both resolved by the app-state banner named in the code comment as future work): the warmup now matches on `Err` and calls `log::warn!`, but `tauri_plugin_log` is registered under `cfg!(debug_assertions)` only (`lib.rs:450`), so in a release build no logger is installed and the warning is a no-op sink — the scenario it exists to catch (a Finder-launched .app whose only Ruby is `/usr/bin/ruby` 2.6) emits nothing; and the warmup thread is spawned a few lines before the log plugin is registered, so even in debug a fast failure can warn before any logger exists.
- **Wasmtime prototype (task #8506, Done):** `ba09207d1` on `worktree-ruby-wasmtime-runner-prototype`. Standalone POC under `wasmtime-poc/` — NOT wired into the Tauri app; no `src-tauri` changes. Satisfies R8 (loads the WASI-standalone `ruby-3.3-wasm32-unknown-wasip1-full`; the npm artifact is documented in-source as unusable due to `rb-js-abi-host`). Uses `epoch_interruption` with a ticker thread, satisfying §4/§9.3 wall-clock enforcement. Fresh `Store` per run, satisfying §5.6. Unreviewed against §7 and §9.2 — the POC does not yet expose a Tauri command or produce `TestResult[]`; integration is the remaining work, not correctness of the runner core. `ba09207d1` commits `compile_bench.rs` and moves `Cargo.toml` to wasmtime v46 with the `parallel-compilation` feature; this is the harness behind the 7.0 s cold / 5.5 s warm compile figures cited in the packaging bullet. Note: `wasmi_bench.rs` is untracked on this branch (not declared as a `[[bin]]`, never built) — this is the reason no wasmi measurement exists; wasmi 0.41 has no bundled WASI host.
- **Packaging assessment:** `6bcc6bcb6` on `worktree-ruby-runtime-packaging-assessment`. Benchmarked; recommendation **reversed to Wasmtime Cranelift**, consistent with §9.3. Independently verified. Key findings: **(a) npm artifact incompatible** — `@ruby/3.3-wasm-wasi` imports `rb-js-abi-host` (58 browser-only JS ABI imports) and cannot be loaded by Wasmtime or wasmi; the native path must use the WASI-standalone build from the ruby.wasm GitHub releases (a different artifact from the one in the pnpm lockfile) — now codified as R8. **(b) Wasmtime epoch interruption** is genuine wall-clock enforcement and satisfies §4/§9.3 directly; the earlier wasmi/fuel proposal is withdrawn. **(c) Per-run cost** 0.26–0.35 s (fresh `Store`+`Instance` per run, satisfying §5.6; `bench.rs` release numbers: compile 26.1 s, per-run 0.313/0.259/0.255 s). **(d) wasmi eliminated**: the 5–20× interpreter factor cited in the assessment is an estimate, not a measurement — `wasmi_bench.rs` was never built (not declared as a `[[bin]]`; wasmi 0.41 has no bundled WASI host). At the estimated factor, per-run becomes 1.3–6.2 s against a 5 s `EXEC_TIMEOUT`. The recommendation stands regardless: Wasmtime wins on epoch/wall-clock grounds under §9.3 independent of wasmi's actual throughput. **RESOLVED — compile-at-first-launch is viable; prebuilt `.cwasm` is optional.** Evidence (packaging doc table, `6bcc6bcb6`): without `parallel-compilation` feature — 31 s (v30) / 44 s (v46) single-threaded; with feature — 7.0 s cold / 5.5 s warm, inside `INIT_TIMEOUT_MS`. Hard requirement: any Wasmtime integration must keep the `parallel-compilation` cargo feature enabled; with `default-features = false` it must be re-added explicitly (the old Cargo.toml omitted it, causing single-threaded compilation). Remaining: `.cwasm` is locked to Wasmtime version and target CPU; `Module::deserialize` validates its own header so a stale cache returns `Err` rather than UB, but a tampered cache carrying a valid header does not, so an integrity check and `app_cache_dir()` placement are still required.
