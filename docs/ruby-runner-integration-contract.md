...
- **Native-process prototype (task #8505):** `6457edaea` on `worktree-ruby-native-runner-prototype` (PR #710). Clippy clean. Single `run_deadline` bounds total wall time at `timeout_ms`; no Rust-side timing constants; process-group containment via `libc::killpg` with pgid > 1 guard; PID race eliminated via `wait-timeout`; `env_clear`; Ruby >= 3.0 version gate; serde-tagged `RubyRunOutcome`. §4, §7, §9.1, §9.1a all satisfied. Predicted fix landed as described (`2d0cf0598`): one `run_deadline` taken at function entry, budget spent by `wait_timeout` then both drains; `PIPE_DRAIN_TIMEOUT` gone (grepped clean). `6457edaea` additionally: exhausted-drain path correctly returns Timeout rather than load_error; Ruby `OnceLock` warmed in the setup closure so the first kata gets its full budget. **ONE open issue:** sandboxing unsolved — the runner executes untrusted kata code with the user's full filesystem, network and exec authority. Two minor items: the setup-closure warmup discards its `Result` (`let _ = get_ruby_binary()`), so a missing-Ruby error is still only discovered as a first-run load_error despite the comment claiming it surfaces at launch; and that warmup runs synchronously before window creation, costing ~70–300 ms of startup.
