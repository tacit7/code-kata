...
- **Native-process prototype (task #8505):** `d71c833c5` on `worktree-ruby-native-runner-prototype` (PR #710). Blockers 1–3 fixed (`recv_timeout` replaces the sleep+kill watchdog; `env_clear` + minimal env; `OnceLock` Ruby ≥3.0 version gate resolving shims via `RbConfig.ruby`). §9.1a satisfied — `timeout_secs`/`output_cap_bytes` are now invoke params sourced from `ruby-exec-core`. §7 serde satisfied — `d71c833c5` replaced hand-rolled JSON with `#[derive(serde::Serialize)]` on `RubyRunOutcome` (tagged `#[serde(tag="status", rename_all="lowercase")]`); TS now does `invoke<RubyRunOutcome>` with no `JSON.parse`. **Still NOT merge-ready — four open issues:**
  ...
  2. **`kill -9` does not reach descendants** — `Command` creates no process group, so a spawned child survives its parent. `Child` is moved into the wait thread, so the kill goes to a raw PID — narrowed from a 5 s window to microseconds, but not eliminated. Verified: SIGKILLing a ruby that had `spawn("sleep 7")` left the sleep alive, reparented to launchd. Violates R3 and R4. Fix: use `.process_group(0)` and kill the negative pgid.
  ...
  4. **Sandboxing unsolved** (unchanged from original blocker 4) — issue 2 above is a concrete demonstration of the cost: the native path cannot currently contain a kata that spawns subprocesses.
