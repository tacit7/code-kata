...
- **Native-process prototype (task #8505):** `d71c833c5` on `worktree-ruby-native-runner-prototype` (PR #710). Blockers 1–3 fixed (`recv_timeout` replaces the sleep+kill watchdog; `env_clear` + minimal env; `OnceLock` Ruby ≥3.0 version gate resolving shims via `RbConfig.ruby`). §9.1a satisfied — `timeout_secs`/`output_cap_bytes` are now invoke params sourced from `ruby-exec-core`. §7 serde satisfied — `d71c833c5` replaced hand-rolled JSON with `#[derive(serde::Serialize)]` on `RubyRunOutcome` (tagged `#[serde(tag="status", rename_all="lowercase")]`); TS now does `invoke<RubyRunOutcome>` with no `JSON.parse`. **Still NOT merge-ready — four open issues:**
  1. **Unbounded hang in the ok branch** — `out_rx.recv()` (`lib.rs:263`) blocks until the stdout pipe hits EOF, which any surviving descendant holds open. Verified: a kata calling `spawn("sleep 8")` exits instantly but the pipe stays open 8 s, past the already-satisfied 5 s gate. Fix: use `recv_timeout` on `out_rx`/`err_rx`.
  2. **Two independent signal defects — both must be fixed:**
     - **(a) `kill -9` does not reach descendants** — `Command` creates no process group, so a spawned child survives its parent. Verified: SIGKILLing a ruby that had `spawn("sleep 7")` left the sleep alive, reparented to launchd. Violates R3 and R4. Fix: use `.process_group(0)` and kill the negative pgid (`kill(-pgid, SIGKILL)`).
     - **(b) Reaped-PID race** — `Child` is moved into the wait thread, so the kill goes to a raw PID after the child has already been reaped. A reaped pgid is just as reusable as a reaped PID; `.process_group(0)` + kill(-pgid) fixes (a) but does NOT close this race. Fix: keep `Child` on the main thread and use the `wait-timeout` crate (`child.wait_timeout(dur)` then `child.kill()`), which cannot signal an already-exited child. Implementing only (a) and ticking this item still ships the race.
  3. **`timeoutSecs` truncation** — TS sends `Math.floor(EXEC_TIMEOUT_MS / 1000)`; any sub-second or non-multiple value silently degrades, and `<1000 ms` yields `recv_timeout(0)`. Pass milliseconds instead.
  4. **Sandboxing unsolved** (unchanged from original blocker 4) — defect 2a above is a concrete demonstration of the cost: the native path cannot currently contain a kata that spawns subprocesses.
